...
- 仅编译 `luci-app-passwall` 与 PassWall 依赖包，跳过 luci-base host tools 以减少构建耗时（如遇构建问题，可在 `.github/workflows/build-installer.yml` 的编译阶段手动补回 `make package/feeds/luci/luci-base/host/compile` 步骤）。
