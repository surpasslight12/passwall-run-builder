...
CONFIG_FILE="$REPO_ROOT/config/openwrt-sdk.conf"
...
Options:
  ...
  --config FILE               Config file to load (default: config/openwrt-sdk.conf)
...
resolve_tag() {
  ...
  if [ -n "$TAG_OVERRIDE" ]; then
    ...
    RAW_TAG=$(resolve_latest_github_release_tag Openwrt-Passwall openwrt-passwall)
...
prefetch_rust_sources() {
  ...
  local -a rust_urls=(
    "https://sources.cdn.openwrt.org/$rust_source"
    "https://sources.openwrt.org/$rust_source"
  )
...
MOCKAPK
  ...
  (
    ...
    cat "$install_log" >&2
    die "Installer smoke test failed"
  }
  grep -q "installed successfully" "$install_log" || die "Smoke installer output missing success marker"
  if [ -f "$PAYLOAD_DIR/TOPLEVEL_PACKAGES" ]; then
    while IFS= read -r pkg; do
      [ -n "$pkg" ] || continue
      grep -q "Installing top-level packages: .*${pkg}" "$install_log" \
        || die "Smoke installer did not include top-level package: $pkg"
